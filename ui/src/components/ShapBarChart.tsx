...
const ShapWaterfallChart = ({ shapResponse }) => {
  ...
  // ✅ Compute cumulative SHAP values correctly for stacking
  ...
  sortedShapValues.forEach((val) => {
    startPositions.push(cumulative);  // This stores where the next bar should start
    cumulative += val;
  });

  // ✅ Prepare chart data with explicit positioning
  const data = {
    ...
    datasets: [
      {
        ...
        borderWidth: 1,
        base: startPositions.slice(0, -1), // ✅ Ensures bars start at correct cumulative position
      },
    ],
  };
